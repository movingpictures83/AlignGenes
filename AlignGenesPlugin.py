...
#Parameters
GENES_OFFSET = 3 #Index offset in dataset where gene expression data starts 
...
def getSamples(dataFilename):
	...
	geneNames = copy.copy(headers[GENES_OFFSET:])
	print("# of Genes", len(geneNames))
	...
	for line in infile:
		...
		if previousSubjectID != '' and previousSubjectID != subjectID:
			...
			sampleSizePerSubject = len(currSubjectSample[geneNames[0]])
		...
		currentAbundancePerTaxa = copy.copy(tokens[GENES_OFFSET:])
		for taxaIndex in range(0,len (geneNames)):
			taxaName = geneNames[taxaIndex]
...
##	print len(samplesPerSubject)
	...
	for subjectID in samplesPerSubject.keys():
		...
		abundanceLevelPerTimepoint = sample[geneNames[0]]
		headers = ['TaxaName']
		for timepoint, abundance in abundanceLevelPerTimepoint:
...
##	outline = 'Reference SampleID' + '\t' + 'Aligned SampleID' + '\t' + 'TaxonError' + '\t' + 'a' + '\t' + 'b' + '\t' + 'alpha' + '\t' + 'beta' + '\t' + 'overlap' + '\t' + 'Taxa Names' + '\t' + 'Alignment Scores (Pearson, Spearman, SSD, R-squared)' + '\n'
	...
	for i in range(0, len(subjectIDs)):
		currentSampleID = copy.copy(list(subjectIDs)[i])
...
#	main(['getAlignmentsIBD_Genes.py', 'ibd_metatranscriptomics.tsv', 'True', 'human_ibd_microbiota_genes_pairwise_subject_ranking.tsv'])
#	main(['getAlignmentsIBD_Genes.py', 'ibd_metatranscriptomics.tsv', 'True', 'human_ibd_microbiota_genes_pairwise_all_subject_ranking.tsv'])
